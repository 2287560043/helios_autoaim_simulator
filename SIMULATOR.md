...
```bash
simulator/run_simulator.sh --tracker all --preset medium_disturb
```

只看参数说明：

```bash
simulator/run_simulator.sh --help
```
...
simulator/run_simulator.sh --tracker simple --standard-mode translate_const --preset light_disturb

# SingerTracker，左右匀速平移 1.0m/s + 轻度扰动
simulator/run_simulator.sh --tracker singer --standard-mode translate_const --preset light_disturb

# SimpleTracker，左右变速平移 -2.0~2.0~-2.0m/s + 中度扰动
simulator/run_simulator.sh --tracker simple --standard-mode translate_var --preset medium_disturb

# SingerTracker，左右变速平移 -2.0~2.0~-2.0m/s + 中度扰动
simulator/run_simulator.sh --tracker singer --standard-mode translate_var --preset medium_disturb
```
...
simulator/run_simulator.sh --tracker top --top-mode spin_const --preset light_disturb

# TopTracker，变速陀螺 20~120~60rpm + 中度扰动
simulator/run_simulator.sh --tracker top --top-mode spin_var --preset medium_disturb

# TopTracker，匀速陀螺 60rpm + 左右匀速平移 0.8m/s + 轻度扰动
simulator/run_simulator.sh --tracker top --top-mode spin_const_translate_const --preset light_disturb

# TopTracker，变速陀螺 30~120~60rpm + 左右变速平移 -1.5~1.5~-1.5m/s + 中度扰动
simulator/run_simulator.sh --tracker top --top-mode spin_var_translate_var --preset medium_disturb

# TopTracker，匀速陀螺 60rpm + 原地高度变化 0~50cm~0（周期5s）+ 轻度扰动
simulator/run_simulator.sh --tracker top --top-mode spin_const_height_var --preset light_disturb

# TopTracker，变速陀螺 20~120~60rpm + 原地高度变化 0~40cm~0（周期5s）+ 中度扰动
simulator/run_simulator.sh --tracker top --top-mode spin_var_height_var --preset medium_disturb
```
...
simulator/run_simulator.sh --tracker top3 --preset medium_disturb
```
...
```bash
# 指定距离和弹速
simulator/run_simulator.sh --tracker top3 --preset medium_disturb --distance-m 7 --bullet-speed 13.5

# 四个 tracker 一起跑综合压力
simulator/run_simulator.sh --tracker all --standard-mode translate_var --top-mode spin_var_translate_var --preset heavy_disturb
```
